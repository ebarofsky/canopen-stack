...
const CO_IF_TIMER_DRV STM32F3xx_TIM16_TimerDriver = {
    DrvTimerInit,
    DrvTimerReload,
    DrvTimerDelay,
    DrvTimerStop,
...
{
    /*
     * This is covered by the autogenerated code
     */
...
static uint32_t DrvTimerDelay(void)
{
    uint32_t current = __HAL_TIM_GET_COUNTER(&htim16);
    uint32_t reload = __HAL_TIM_GET_AUTORELOAD(&htim16);
...
static void DrvTimerReload(uint32_t reload)
{
    ...
    __HAL_TIM_SET_AUTORELOAD(&htim16, reload);
}
